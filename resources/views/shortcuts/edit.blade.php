...
@section('title', __('Edit Pintasan'))

@section('content')
    <div class="mb-4">
        <h1 class="text-2xl font-semibold text-gray-800">{{ __('Edit Pintasan') }}</h1>
    </div>

    <div class="bg-white border border-gray-200 rounded-lg p-6">
        <form action="{{ route('shortcuts.update', $shortcut) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="title" class="block mb-2 text-sm font-medium text-gray-900">{{ __('Judul') }}</label>
                <input type="text" name="title" id="title" value="{{ old('title', $shortcut->title) }}" required
            ...
            <div class="mb-4">
                ...
                <input type="url" name="url" id="url" value="{{ old('url', $shortcut->url) }}" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                @error('url')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            @include('shortcuts.partials.icon-picker', ['value' => old('icon', $shortcut->icon)])
